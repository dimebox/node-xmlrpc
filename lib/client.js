 ...
 *   - {String} responseEncoding  - (optional) Specifies an encoding for the response (ex: ISO-8859-1). Default to utf8
 *   - {String} proxyUrl          - URL to proxy server
...
function Client(options, isSecure) {
  ...
  var proxyUrl = options.proxyUrl || process.env.http_proxy || process.env.https_proxy

  if (proxyUrl) {
    options.agent = new require('https-proxy-agent')(proxyUrl)
  }
}
