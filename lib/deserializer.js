  ...
  if (!this.error) {
    if (this.type === null || this.marks.length) {
      ...
      var createFault = function(fault) {
        ...
        error.faultString = iconv.encode(iconv.decode(new Buffer(fault.faultString), that.encoding), 'utf8').toString()
        return error
      }
...
Deserializer.prototype.onClosetag = function(el) {
  var data = this.data.reduce(function (buf, x) {
    return Buffer.concat([buf, new Buffer(x, "binary")])
  }, new Buffer("", "binary"))

  data = iconv.encode(iconv.decode(new Buffer(data), this.encoding), 'utf8').toString()
